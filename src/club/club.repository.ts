...
export class ClubRepository{
    ...
    async getClubMemberStatus(clubId: number, userId: number): Promise<ClubMemberStatus | null> {
    ...
    async countClubPendingMembersById(clubId: number, userIds: number[]): Promise<number> {
        return this.prisma.clubMember.count({
            where: {
                clubId: clubId, 
                userId: { in: userIds }, 
    ...
    async approveApplicants(clubId: number, data: ApproveApplicantsData): Promise<void> {
    ...
    async rejectApplicants(clubId: number, data: RejectApplicantsData) : Promise<void>{
        await this.prisma.clubMember.deleteMany({
            where: {
                clubId: clubId, 
                userId: { in: data.userIds }, 
    ...
    async updateClubHost(clubId: number, hostId: number, nextHostId: number): Promise<any> {
